...
import { DonationPaymentInfo } from '../src/donation-info/donation-payment-info';
...
describe('Donation Payment Info', () => {
  ...
  it('returns 0 during the amount calculation if the amount is NaN', async () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const result = DonationPaymentInfo.calculateTotal('a' as any, false);
    expect(result).to.equal(0);
  });

  it('returns 0 during the fee calculation if the amount is NaN', async () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const result = DonationPaymentInfo.calculateFeeAmount('a' as any);
    expect(result).to.equal(0);
  });
});
